...
| **3 — Schedule & Atomic Shift Core** | **In progress** | Checkpoint 1 done: schema/migrations, RRULE-based recurrence generation, collection-point/schedule-template/session CRUD, atomic version-gated `claimShift`/`releaseShift`, and a parent-facing Schedule web page — all tested (18 new API tests incl. a real 10-way concurrency race) and manually verified in-browser in both locales. Admin UI, Home-workspace revamp, and template-edit/bulk-edit are explicitly deferred to follow-up checkpoints. |
...
3. Stage 3's first checkpoint (backend atomic-shift core + parent-facing Schedule page) is done — see the Stage 3 Progress note below. Next: either the admin schedule/collection-point management UI, the Home-workspace revamp, or template-edit/bulk-edit — all deferred from checkpoint 1, not yet prioritized against each other.
...
- [ ] Implement recurrence parsing/generation using a tested RRULE library. Generate the configured horizon idempotently, preserve past/in-progress records, and prevent cancelled sessions from regenerating shifts.
  - Initial generation is done: the `rrule` package parses the recurrence rule, `apps/api/src/lib/recurrence.ts` (unit-tested) turns it into occurrence instants, and template creation generates the full horizon in one transaction. A `(templateId, startsAt)` unique constraint makes creation idempotent against duplicate sessions. **Not yet built:** the actual "regenerate on template edit" flow this item also calls for — there's no template-edit endpoint at all yet (see the next item). Deferred to the next checkpoint by explicit scope decision, since it's materially more complex (must preserve past/in-progress sessions untouched) and the atomic-claim core was the priority to prove out first.
...
- [ ] Build collection-point management with address, optional coordinates, valid direction type, player-assignment validation, and visibility of the full assigned player list before a driver claims.
  - Backend CRUD done and tested (`apps/api/src/routes/collection-points.ts`): create/list/update/delete, GPS coordinates optional and clearable, delete blocked with a `409` if the point already has scheduled shifts (audit-integrity guard, same principle as the last-admin safeguard). **Not yet built:** any admin UI. The parent-facing Schedule page shows a player *count* per collection point, not the full name list (no player-lookup endpoint exists yet — deferred to whichever checkpoint builds the admin roster/assignment UI that will need it anyway).
...
- [ ] Build the web Home workspace: a concise next-action strip, personal assignments, pending swap count, help-needed shifts, and compact personal coverage summary. Avoid a marketing landing page.
  - Not started this checkpoint — explicitly out of scope by the agreed checkpoint split (this slice covered the backend core plus the Schedule view only). The existing Home page gained a nav link to Schedule but is otherwise unchanged.
...
- [ ] Build admin schedule/collection-point screens as dense operational tables and side panels rather than stacked decorative cards. Include validation summaries before saving changes.
  - Not started — no admin UI shipped this checkpoint (see the collection-point and session-management items above); the backend commands it would call already exist and are tested.
