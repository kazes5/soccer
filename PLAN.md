...
- [ ] Implement invite-only onboarding: admin creates expiring invite, recognized invitee verifies phone/email, completes profile and linked-player details, then reaches the team-aware home view.
  - `POST /teams` (bootstrap a team + its first admin) and `POST /teams/:teamId/invites` (admin-only, audited) are implemented and tested. Invite *acceptance* (recognized invitee redeems a code, completes their profile/players, lands on a team-aware home view) is not built yet — that's the next concrete slice, along with the web onboarding UI.
...
**Progress (2026-08-09):**
- **Status:** In progress — identity/membership data model and the team-bootstrap/invite/OTP-login backend vertical slice are built and tested; onboarding UI, i18n, and the design system haven't started.
- **Evidence:** `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (21 tests across all packages), and `pnpm build` all pass from a clean checkout, against a real Postgres via `docker compose up -d`. New API surface: `POST /teams`, `GET /teams/:teamId`, `POST /teams/:teamId/invites`, `POST /auth/otp/request`, `POST /auth/otp/verify`.
- **Decisions made during this slice:**
  ...
  - `packages/contracts/src/index.ts` is now a barrel over per-domain files (`health.ts`, `enums.ts`, `team.ts`, `user.ts`, `invite.ts`, `auth.ts`) instead of one flat file, since Stage 2 added enough schemas to need it.
...
- **Next concrete action:** Invite-acceptance endpoint (redeem code → create user/profile/players → first session) and the onboarding UI, or start the i18n/design-system track in parallel.
