...
- [ ] Implement `User`, `Team`, `TeamMember`, `Player`, invite, OTP challenge, session, browser subscription, notification-preference, and audit tables with indexes and foreign keys.
  - `User`, `Team`, `TeamMember`, `Player`, `PlayerParent`, `Invite`, `OtpChallenge`, `Session`, `NotificationPreference`, and `AuditLog` are modeled in `apps/api/prisma/schema.prisma` and migrated (`20260806223652_identity_membership_core`), with indexes/unique constraints per PLAN.md's architecture decisions (role on `team_members`, IANA `timezone` on `team`). No `browser subscription` (push registration) table yet — that lands with the browser-push notification work.
- [ ] Implement invite-only onboarding: admin creates expiring invite, recognized invitee verifies phone/email, completes profile and linked-player details, then reaches the team-aware home view.
  - `POST /teams` (bootstrap a team + its first admin), `POST /teams/:teamId/invites` (admin-only, audited), `GET /invites/:code` (preview team name before accepting), and `POST /invites/:code/accept` (atomic accept — creates the parent's profile and linked players, or joins an existing multi-team user to the new team, then the invite is redeemed via a normal OTP login) are implemented and tested. What's left is purely the web onboarding UI — the backend contract for the full journey is done.
- [ ] Enforce OTP expiry, per-phone and per-IP rate limits, hashed/revocable sessions, secure cookies, CSRF protection where applicable, and immediate deactivation on removal.
  - `POST /auth/otp/request` / `POST /auth/otp/verify` implement OTP expiry (`OTP_TTL_MINUTES`), per-user rate limiting (`OTP_MAX_REQUESTS_PER_HOUR`) and max verify attempts (`OTP_MAX_VERIFY_ATTEMPTS`), hashed OTP codes and session tokens (SHA-256, timing-safe compare), and session expiry (`SESSION_TTL_DAYS`). Not yet done: per-IP rate limiting, a logout/revoke-session endpoint (schema supports `revokedAt` but nothing sets it), secure-cookie delivery (sessions currently return a bearer token in the response body — reasonable for a backend-only slice, but revisit before wiring up the web client), CSRF, and deactivation-on-removal (no user-removal endpoint exists yet).
...
**Progress (2026-08-09):**
- **Status:** In progress — the full backend contract for team bootstrap, invite issuance/acceptance, and OTP login is built and tested. Onboarding UI, i18n, and the design system haven't started.
- **Evidence:** `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test` (28 tests across all packages), and `pnpm build` all pass from a clean checkout, against a real Postgres via `docker compose up -d`. API surface: `POST /teams`, `GET /teams/:teamId`, `POST /teams/:teamId/invites`, `GET /invites/:code`, `POST /invites/:code/accept`, `POST /auth/otp/request`, `POST /auth/otp/verify`.
- **Decisions made during this slice:**
  ...
  - Sessions are bearer tokens (SHA-256 hash stored, raw token returned once on login), not cookies — simplest correct option for a backend-only slice with no web client wired up yet. Revisit cookie vs. bearer (and CSRF) when the web login UI is built.
  - `packages/contracts/src/index.ts` is now a barrel over per-domain files (`health.ts`, `enums.ts`, `team.ts`, `user.ts`, `player.ts`, `invite.ts`, `auth.ts`) instead of one flat file, since Stage 2 added enough schemas to need it.
  - Invite acceptance identifies the contact (phone or email) from the *invite record*, not from the request body, so an accepted invite can't be hijacked into registering a different phone/email than the one the admin actually invited. If a user with that contact already exists (multi-team household), acceptance joins them to the new team instead of creating a duplicate `User`.
  - The invite `pending → accepted` transition is a conditional `updateMany` inside the transaction (count-checked), the same compare-and-set pattern PLAN.md's architecture decisions call for on shift claims — it closes the same double-accept race.
- **Blocker or risk:** None blocking. The audit-log schema requires a non-null `teamId`, so "login attempt by a non-registered user" (a `CLAUDE.md` §5 loggable event) currently isn't audit-logged since there's no team to attribute it to — flagging as a known gap rather than a silent omission.
- **Next concrete action:** Web onboarding UI (invite landing page → registration form → team-aware home), or start the i18n/design-system track in parallel. Smaller open items: per-IP OTP rate limiting, a logout/session-revoke endpoint, and enabling GitHub branch protection (`ci` required status check) on `main`, which is still off.
