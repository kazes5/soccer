...
| **2 — Identity/Membership/i18n/Design** | **In progress** | Backend + onboarding UI loop, i18n foundation, and design tokens/accessible primitives all built and verified (two checkpoints not yet merged); backend hardening (rate limits, cookie sessions, member management) also built, not yet merged |
...
1. Design tokens + accessible primitives checkpoint is code-reviewed and ready for PR (`stage2-design-tokens` branch) — see Progress note below.
2. Backend hardening checkpoint (per-IP rate limit, secure-cookie/CSRF session transport, member management) is on a separate branch/PR (`stage2-backend-hardening`, PR #8), open but not yet merged. Once both land, the frontend should be re-pointed at the cookie-based session flow from that PR (this checkpoint was built against the still-`localStorage`-based `main`, since PR #8 hadn't merged).
3. After both merge: native-speaker Hebrew review, and a staging deploy — the two remaining Stage 2 exit criteria that can't be satisfied from this environment.
4. Branch protection on `main` is done (verified 2026-08-09 — scoped correctly to `main` only, after an initial misconfiguration that accidentally protected every branch was caught and fixed).
...
- **Status:** Done for this checkpoint. All four remaining Stage 2 frontend checklist items (tokens, Heebo font, accessible primitives, visual QA) are built, tested, code-reviewed with every finding addressed, and manually verified end-to-end in a real browser across desktop/tablet/mobile and both locales. Built on top of `main` (the `stage2-backend-hardening` PR hasn't merged yet), so this still uses the `localStorage`-based session from the onboarding checkpoint, not the cookie-based session from that other PR — reconciling the two is next, once both merge.
...
- **Blocker or risk:** None blocking. Known, explicitly-deferred gaps: (1) still built against the `localStorage`-session `main`, not the cookie-session `stage2-backend-hardening` branch — needs reconciling once both merge; (2) no automated accessibility scan (axe, screen-reader smoke test) has run — the primitives were built with semantic roles/`aria-live`/focus rings/`aria-describedby` and manually spot-checked, but that's Stage 6's job per the Exit Criteria below, not this checkpoint's; (3) the dev-mode hydration-mismatch warning some browsers show on this app is caused by a `data-testim-*` attribute a browser extension injects into `<body>` before React hydrates — confirmed absent on a production build with no such extension present, not an app bug.
- **Next concrete action:** Commit and open the PR for this checkpoint. After that: merge both open Stage 2 PRs (this one and `stage2-backend-hardening`), reconcile the frontend onto the cookie-based session flow, then get a native speaker to review the Hebrew catalog and stand up a staging environment — the two remaining Stage 2 exit criteria.
