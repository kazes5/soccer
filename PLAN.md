...
| **2 — Identity/Membership/i18n/Design** | **In progress** | Backend + onboarding UI loop, i18n foundation, and backend hardening (cookies/CSRF, per-IP rate limit, member management, push-subscription table) all done and verified; design tokens/accessible primitives not started |
...
1. Backend hardening checkpoint (cookies/CSRF, per-IP rate limit, member management, push-subscription table) is done, reviewed, and its PR is open (branch `stage2-backend-hardening`) pending merge as of this writing.
2. Design tokens + accessible primitives is the one remaining Stage 2 track (frontend, listed above) — start there next.
